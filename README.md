...
const response = await axios.post('https://api.bottyfier.com/v1/check', {
...
```bash
curl -X POST https://api.bottyfier.com/v1/check \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"email":"user@example.com","ip":"1.2.3.4","fingerprint":"browser-fingerprint-string"}'
```
...
response = requests.get(
    'https://api.bottyfier.com/ip/8.8.8.8',
    params={'key': 'YOUR_API_KEY'}
)
data = response.json()

if data['isVpn'] or data['isDatacenter'] or data['isBlacklisted']:
    print('Suspicious IP detected')
else:
    print('IP is safe')

print('Country:', data['ipApiData']['country'])
print('ISP:', data['ipApiData']['isp'])
```
...
if ($data['isVpn'] || $data['isDatacenter'] || $data['isBlacklisted']) {
    echo 'Suspicious IP detected';
} else {
    echo 'IP is safe';
}

echo 'Country: ' . $data['ipApiData']['country'] . PHP_EOL;
echo 'ISP: ' . $data['ipApiData']['isp'] . PHP_EOL;
?>
```
...
type IPResponse struct {
    ...
    IsDatacenter  bool
    IsBlacklisted bool
    IPAPIData     map[string]interface{}
}
...
func main() {
    ...
    if data.IsVPN || data.IsDatacenter || data.IsBlacklisted {
        fmt.Println("Suspicious IP detected")
    } else {
        fmt.Println("IP is safe")
    }

    fmt.Println("Country:", data.IPAPIData["country"])
    fmt.Println("ISP:", data.IPAPIData["isp"])
}
```
...
All API requests require authentication using your API key. Include it as a query parameter or in the Authorization header.
